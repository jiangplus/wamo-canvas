...
const getAvatarUrl = (seed) => `https://api.dicebear.com/7.x/notionists/svg?seed=${seed}`;
...
export default function App({ canvasId, onBack }) {
  ...
  const elements = useMemo(() => {
    ...
    return canvas.elements.map(el => ({
      ...
      comments: el.comments?.map(c => ({
        ...
        avatar: getAvatarUrl(c.author?.[0]?.email?.split('@')[0] || 'unknown'),
      })) || [],
    }));
  }, [canvas]);
  ...
  const updateElement = useCallback((elementId, updates) => {
    ...
    const { creatorName, avatar, comments, creator, ...schemaUpdates } = updates;
  ...
  const addConnection = useCallback((fromId, toId) => {
    ...
    db.transact([
      tx.connections[connectionId].update({
        text: '',
        createdAt: Date.now(),
      })
        .link({ canvas: canvasId })
  ...
  const addComment = useCallback((elementId, text) => {
  ...
  useEffect(() => {
    ...
    [...picturePool.public, ...picturePool.private].forEach(i => {
      if (!drawerImageAngles[i.id]) imgAngles[i.id] = generateSmallRandomAngle(8);
    });
  ...
  }, [selectedId, elements, clipboard, editingTextId, newCommentTargetId, editingCommentId, lassoState, deleteElement, addElement]);
  ...
  const handleElementMouseDown = (e, elId, type) => {
    ...
    if (activeTool === 'connect') {
      if (!connectFrom) { setConnectFrom(elId); setSelectedId(elId); }
    ...
    dragRef.current = {
      id: elId, type, startX: e.clientX, startY: e.clientY,
  ...
  const handleMouseMove = (e) => {
    ...
    if (interactionState === 'panning') setViewport(p => ({ ...p, x: dragRef.current.initialViewport.x + dx / p.scale, y: dragRef.current.initialViewport.y + dy / p.scale }));
      ...
      const { id: elId, initialW, initialScale, centerX, centerY, initialDist } = dragRef.current;
      const el = elements.find(i => i.id === elId);
      ...
      if (el.type === 'text') updateElement(elId, { scale: Math.max(0.5, initialScale * ratio) });
      else updateElement(elId, { width: Math.max(80, initialW * ratio) });
    }
    else if (interactionState === 'rotating') {
      const { id: elId, startX, startY, initialRot, centerX, centerY } = dragRef.current;
  ...
  const handleMouseUp = (e) => {
    ...
    if (draggedFromDrawer) {
      ...
      addElement({
        ...
        shape: null,
        style: draggedFromDrawer.type === 'text' ? { ...previewTextStyle } : null,
      });
  ...
  const handleFileUpload = (e) => {
    Array.from(e.target.files || []).forEach(f => {
      ...
      r.onload = (ev) => {
        const imgId = id();
        setPicturePool(p => ({ ...p, [activeTab]: [...p[activeTab], { id: imgId, url: ev.target.result }] }));
        setDrawerImageAngles(p => ({ ...p, [imgId]: generateSmallRandomAngle(8) }));
      };
      r.readAsDataURL(f);
    });
    e.target.value = '';
  ...
  const handleCanvasDrop = (e) => {
    ...
    r.onload = (ev) => {
      const wp = getWorldCoords(e.clientX, e.clientY);
      addElement({ type: 'image', content: ev.target.result, x: wp.x - 110, y: wp.y - 100, width: DEFAULT_ELEMENT_WIDTH, rotation: 0, shape: null });
    };
    r.readAsDataURL(f);
  };
  ...
  const toolbarActions = (el) => ({
    ...
    onShuffle: () => !el.isLocked && updateElement(el.id, { style: generateMagazineStyle() }),
    ...
    onMoveUpLayer: () => { updateElement(el.id, { zIndex: maxZIndex + 1 }); },
    onDuplicate: () => {
      addElement({
        type: el.type,
        content: el.content,
    ...
    onDelete: () => { if (!el.isLocked) deleteElement(el.id); },
  });
        ...
        <div className="canvas-content absolute" style={{ transformOrigin: '0 0', transform: `scale(${viewport.scale}) translate(${viewport.x}px, ${viewport.y}px)`, transition: isAnimating ? 'transform 0.4s cubic-bezier(0.4, 0, 0.2, 1)' : 'none' }}>
          ...
          {sortedElements.map(el => (
            <CanvasElement
              ...
              onSubmitEdit={t => { if (t.trim()) { updateElement(el.id, { content: t.trim() }); } setEditingTextId(null); }}
